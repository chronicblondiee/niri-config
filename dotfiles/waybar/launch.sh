...
if [ ! -f $HOME/.config/ml4w/settings/waybar-disabled ]; then
    if [ "$XDG_CURRENT_DESKTOP" = "Hyprland" ]; then
        ...
        # Use niri-specific config if it exists, otherwise fall back to default
        niri_config="config-niri"
        if [ ! -f ~/.config/waybar/themes${arrThemes[0]}/$niri_config ]; then
            niri_config="$config_file"
        fi
        waybar -c ~/.config/waybar/themes${arrThemes[0]}/$niri_config -s ~/.config/waybar/themes${arrThemes[1]}/$style_file &
    else
